/**
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 * FORMCYCLE FLEXIBLE MULTIPLE UPLOAD - CODE INJECTION (Option 4)
 * ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 *
...
(function() {
    ...
    const CONFIG = {
        ...
        DEBUG: true,
        PATCH_DELAY: 200,  // ms zu warten bevor patchen
    ...
    // ============================================
    // SETTER METHODE (Global verfügbar machen)
    // ============================================
    window.updateConfig = function(key, value) {
        if (CONFIG.hasOwnProperty(key)) {
            CONFIG[key] = value;
            if (CONFIG.DEBUG) console.log(`Config updated: ${key} =`, value);
        } else {
            console.warn(`Key "${key}" existiert nicht in CONFIG.`);
        }
    };

    /**
     * Macht die Initialisierung global verfügbar
     */
    window.initMultiUpload = function() {
        log('Manueller Init-Aufruf von extern gestartet...');
        return init(); 
    };

    // Optional: Automatischer Start nach Delay (falls gewünscht)
    //setTimeout(window.initUploadPatch, CONFIG.PATCH_DELAY);


    // ============================================
    // PER-ELEMENT KONFIGURATION (aus HTML-Attributen)
    // ============================================

    /**
     * Liest feldspezifische Konfiguration aus HTML-Attributen des Elements.
     * Fallback auf globale CONFIG-Werte wenn kein Attribut gesetzt.
     *
     * Unterstützte Attribute am input-Element:
     *   data-max-file-size    Maximale Dateigröße in Bytes    (z.B. 5242880 = 5 MB)
     *   data-max-total-size   Maximale Gesamtgröße in Bytes   (z.B. 52428800 = 50 MB)
     *   data-max-files        Maximale Anzahl Dateien         (z.B. 5)
     *
     * Beispiel:
    ...
    function getFieldConfig($field) {
        function attr(name, defaultValue) {
            const val = $field.attr(name);
            if (val === undefined || val === null || val === '') return defaultValue;
            return parseInt(val, 10);
        }

        return {
            MAX_FILE_SIZE:  attr('data-max-file-size',  CONFIG.MAX_FILE_SIZE),
            MAX_TOTAL_SIZE: attr('data-max-total-size', CONFIG.MAX_TOTAL_SIZE),
            MAX_FILES:      attr('data-max-files',      CONFIG.MAX_FILES),
            // Folgende Werte sind nicht per HTML-Attribut konfigurierbar –
            // sie werden ausschließlich aus der globalen CONFIG bezogen:
            DEBUG:                   CONFIG.DEBUG,
            PATCH_DELAY:             CONFIG.PATCH_DELAY,
            PATCH_STRATEGY:          CONFIG.PATCH_STRATEGY,
            SPECIFIC_FIELDS:         CONFIG.SPECIFIC_FIELDS,
            RESPECT_METADATA_LIMITS: CONFIG.RESPECT_METADATA_LIMITS,
        };
    }    
    ...
    function shouldPatchField($field) {
        ...
        switch (fieldConfig.PATCH_STRATEGY) {
            case 'all':
                return {
                    patch: true,
            ...
            case 'specific':
                const isSpecific = fieldConfig.SPECIFIC_FIELDS.includes(fieldId);
                return {
                    patch: isSpecific,
                    reason: isSpecific
    ...
    function patchFormcycleUpload() {
        ...
        manager.getUpload = function(field) {
            ...
            // ============================================
            // FELDSPEZIFISCHE KONFIGURATION LADEN
            // ============================================
            ...
            if (fieldConfig.RESPECT_METADATA_LIMITS && window.XFC_METADATA && XFC_METADATA.limits) {
    ...
    function validateFiles(files, $field) {
        ...
        // Hole feldspezifische Limits (HTML-Attribute haben Vorrang vor globalem CONFIG)
        const fieldConfig = getFieldConfig($field);
        ...
        files.forEach((file, index) => {
            ...
            // Prüfe Dateigröße
            if (file.size > fieldConfig.MAX_FILE_SIZE) {
        ...
        if (validFiles.length > fieldConfig.MAX_FILES) {
            return {
                valid: false,
                allInvalid: false,
                errors: [`Maximum von ${fieldConfig.MAX_FILES} Dateien überschritten (${validFiles.length} gültige Dateien)`],
                validFiles: [],
                invalidFiles: files
            };
        }
        ...
        if (totalSize > fieldConfig.MAX_TOTAL_SIZE) {
            return {
                valid: false,
                allInvalid: false,
                errors: [`Gesamtgröße zu groß: ${formatSize(totalSize)} (max ${formatSize(fieldConfig.MAX_TOTAL_SIZE)})`],
                validFiles: [],
                invalidFiles: files
            };
        }
    ...
    /**
     * Versteckt alle Elemente, die auf die gegebenen CSS-Selektoren passen,
     * innerhalb von `containerEl` – sofort falls schon vorhanden, sonst per
     * MutationObserver sobald sie ins DOM eingefügt werden.
     *
     * @param {Element}  containerEl  Natives DOM-Element, das beobachtet wird
     * @param {...string} selectors   Beliebig viele CSS-Selektoren
     */
    function hideWhenPresent(containerEl, ...selectors) {
        const $container = $(containerEl);

        function hideMatches() {
            selectors.forEach(sel => $container.find(sel).hide());
        }

        function allFound() {
            return selectors.every(sel => $container.find(sel).length > 0);
        }

        // Sofort verstecken falls bereits im DOM
        hideMatches();
        if (allFound()) return;

        log('⏳ Warte auf Formcycle-UI-Elemente im Container:', containerEl.id || containerEl);

        const observer = new MutationObserver(() => {
            hideMatches();
            if (allFound()) {
                observer.disconnect();
                log('✅ Formcycle-UI-Elemente versteckt in:', containerEl.id || containerEl);
            }
        });

        observer.observe(containerEl, { childList: true, subtree: true });

        // Sicherheits-Timeout: Observer nach 30 s stoppen
    ...
    function createUI() {
        ...
        $fields.each(function() {
            ...
            // Diese werden von Formcycle asynchron nach dem Input gerendert,
            // daher per MutationObserver warten bis sie im DOM erscheinen.
            hideWhenPresent($container[0], '.xm-upl-wrapper', '.xm-upl-input-wrapper');
            ...
            // Feldspezifische Konfiguration für UI-Anzeige
            const fieldConfig = getFieldConfig($field);

            const strategyText = fieldConfig.PATCH_STRATEGY === 'all' ? 'Alle Felder' :
                               fieldConfig.PATCH_STRATEGY === 'custom-upload' ? 'Custom-Upload' :
                               'Spezifische Felder';
            ...
            const sizeInfo = maxSizeText
                ...
                : `${formatSize(fieldConfig.MAX_FILE_SIZE)} pro Datei`;
            const fileNameLengthInfo = maxFileNameLength
                ? `Dateiname max ${maxFileNameLength} Zeichen`
                : '';

            const detailsLine = [formatInfo, sizeInfo, `Max ${fieldConfig.MAX_FILES} Dateien`, fileNameLengthInfo]
                .filter(x => x)
                .join(' • ');
    ...
    function updateFileList($field, $fileListContent, $emptyState, $btnClear, $stats) {
        ...
        $stats.html(`
            ...
            Noch <strong>${fieldConfig.MAX_FILES - files.length}</strong> erlaubt
        `).show();
